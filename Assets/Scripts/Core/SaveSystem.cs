...
namespace NexusPrime.Core
{
    public static class SaveSystem
    {
        ...
        public static void SaveGame(string saveName, GameManager gameManager)
        {
            try
            {
                ...
                foreach (SelectableUnit unit in gameManager.GetAllUnits())
                {
                    ...
                    unitData.health = unitStats != null ? unitStats.currentHealth : 0f;
                ...
                foreach (Building building in gameManager.GetAllBuildings())
                {
                ...
                // Save resources (serializable list; JsonUtility doesn't serialize Dictionary)
                saveData.resourceList = new List<ResourceSaveEntry>();
                if (gameManager.resourceManager != null)
                {
                    foreach (var kv in gameManager.resourceManager.resources)
                    {
                        saveData.resourceList.Add(new ResourceSaveEntry
                        {
                            key = kv.Key.ToString(),
                            value = kv.Value.amount
                        });
                    }
                }
    ...
    public class GameSaveData
    {
        ...
        public List<ResourceSaveEntry> resourceList;
